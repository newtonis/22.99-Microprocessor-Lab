...
void testSPI(uint8_t SPI_n){
	...
	uint16_t data_received[3] = {0,0,0};
	spi_command command;
	...
	data_received[0]=masterWriteReadByte(SPI_n, &command, data[0]);
	data_received[1]=masterWriteReadByte(SPI_n, &command, data[1]);
	data_received[2]=masterWriteReadByte(SPI_n, &last_command, data[2]);
...
uint16_t masterWriteRead(uint8_t SPI_n, spi_command *command, uint16_t data)
{
