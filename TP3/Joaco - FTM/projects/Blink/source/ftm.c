...
void SetInputCaptureOrOutputCompare(FTM_Type * module_pnt ,uint8_t ch_num,bool isOutputCompare){
	...
	if(isOutputCompare){
		module_pnt->CONTROLS[ch_num].CnSC |= FTM_CnSC_MSA_MASK;
	}else{
		module_pnt->CONTROLS[ch_num].CnSC &= ~ FTM_CnSC_MSA_MASK;
	}
}
...
void setModeConfig(FTM_Type * module_pnt, uint8_t ch_num, uint8_t mode){
	...
	bool elsb = (mode & FTM_CnSC_ELSB_MASK) == FTM_CnSC_ELSB_MASK ;
	bool elsa = (mode & FTM_CnSC_ELSA_MASK) == FTM_CnSC_ELSA_MASK ;
